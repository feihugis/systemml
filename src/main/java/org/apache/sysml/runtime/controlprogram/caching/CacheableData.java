...
public abstract class CacheableData<T extends CacheBlock> extends Data
{
          ...
          if( isEmpty(true) && _data==null )
          {
            try
            {
              ...
              if( getRDDHandle()==null || getRDDHandle().allowsShortCircuitRead() )
              {
                ...
                //read cacheable data from hdfs
                _data = subBlobFromHDFS(_hdfsFileName, ixRange);
		...
		throws IOException 
	{
        ...
        protected T subBlobFromHDFS(String fname, IndexRange ixRange)
            throws IOException, DMLRuntimeException
        {
          MatrixFormatMetaData iimd = (MatrixFormatMetaData) _metaData;
          MatrixCharacteristics mc = iimd.getMatrixCharacteristics();
          return subBlobFromHDFS(fname, mc.getRows(), mc.getCols(), ixRange);
        }

	protected abstract T readBlobFromHDFS(String fname, long rlen, long clen) 
		throws IOException;

        protected T subBlobFromHDFS(String fname, long rlen, long clen, IndexRange ixRange)
            throws IOException, DMLRuntimeException
        {
