...
public class MatrixObject extends CacheableData<MatrixBlock>
{
		...
		throws IOException
	{
        ...
        @Override
        protected MatrixBlock subBlobFromHDFS(String fname, long rlen, long clen, IndexRange ixRange)
            throws IOException
        {
                MatrixFormatMetaData iimd = (MatrixFormatMetaData) _metaData;
                MatrixCharacteristics mc = iimd.getMatrixCharacteristics();
                ...
                //read matrix and maintain meta data
                double sparsity = (mc.getNonZeros() >= 0 ? ((double)mc.getNonZeros())/(mc.getRows()*mc.getCols()) : 1.0d);
                MatrixBlock newData = DataConverter.subMatrixFromHDFS(fname, iimd.getInputInfo(), rlen, clen,
                                                                       mc.getRowsPerBlock(), mc.getColsPerBlock(), sparsity, getFileFormatProperties(),
                                                                      ixRange);
                setHDFSFileExists(true);
